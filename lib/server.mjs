...
function openPicker(sid) {
  ...
  grid.innerHTML = CHAR_NAMES.map((name, i) => {
    ...
    const m = isLocked ? '<div style="width:32px;height:32px;display:flex;align-items:center;justify-content:center;"><svg viewBox="0 0 16 16" fill="none" stroke="var(--gray-400)" stroke-width="1.2" style="width:20px;height:20px"><rect x="3" y="7" width="10" height="7" rx="1.5"/><path d="M5 7V5a3 3 0 016 0v2"/></svg></div>' : mascot("waiting", "picker-" + i, i);
...
const UNLOCK_THRESHOLDS = [
  ...
  { idx: 1,  minutes: 10,    name: "강아지", desc: "10분 사용 달성! 충성스러운 친구가 왔어요" },
  { idx: 2,  minutes: 30,    name: "토끼",   desc: "30분 사용 달성! 수줍은 친구가 나타났어요" },
  { idx: 3,  minutes: 60,    name: "곰",     desc: "1시간 사용 달성! 듬직한 친구가 합류했어요" },
  { idx: 4,  minutes: 120,   name: "펭귄",   desc: "2시간 사용! 시원한 친구가 도착했어요" },
  { idx: 5,  minutes: 180,   name: "호랑이", desc: "3시간 사용! 용맹한 친구를 만났어요" },
  { idx: 6,  minutes: 300,   name: "사자",   desc: "5시간 사용! 왕의 귀환이에요" },
  { idx: 7,  minutes: 480,   name: "알파카", desc: "8시간 사용! 뽀글뽀글 친구가 왔어요" },
  { idx: 8,  minutes: 720,   name: "여우",   desc: "12시간 사용! 영리한 친구가 나타났어요" },
  { idx: 9,  minutes: 1200,  name: "햄스터", desc: "20시간 사용! 바쁜 친구가 합류했어요" },
  { idx: 10, minutes: 1800,  name: "부엉이", desc: "30시간 사용! 현명한 친구를 만났어요" },
...
function getUnlockedSet() {
  try { return new Set(JSON.parse(localStorage.getItem(UNLOCKED_KEY) || "[0]")); } catch { return new Set([0]); }
}
...
function renderCollectionBar(minutes, unlocked) {
  ...
  const dots = UNLOCK_THRESHOLDS.map(t => {
    ...
    const m = isUnlocked ? miniCollectionMascot(t.idx) : '<svg viewBox="0 0 16 16"><rect x="4" y="4" width="8" height="8" rx="2" fill="var(--gray-300)"/></svg>';

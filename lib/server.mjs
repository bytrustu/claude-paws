...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .unlock-overlay.open .unlock-card {
  ...
  .unlock-mascot-wrap {
    ...
    margin: 0 auto 16px;
    position: relative;
    animation: unlock-pop 0.6s 0.2s cubic-bezier(0.34,1.56,0.64,1) both;
  }
  .unlock-mascot-wrap .mascot {
    width: 80px;
    height: 80px;
  }
  .unlock-mascot-wrap .mascot svg {
    width: 80px;
    height: 80px;
  }
  ...
  .toast-icon.done { background: var(--green-bg); color: var(--green); }
  .toast-icon.alert { background: var(--purple-bg); color: var(--purple); }
  ...
  .collection-dot.locked {
    background: var(--gray-100);
    opacity: 0.4;
  }
  .collection-dot.unlocked {
...
<div class="wrap">
  <header class="header">
    ...
    <div class="header-actions">
      ...
      <div class="notif-toggle" id="notifToggle" title="macOS 알림 on/off">
        <div class="notif-toggle-track" id="notifTrack">
          <div class="notif-toggle-thumb"></div>
        </div>
        <span class="notif-toggle-label" id="notifLabel">알림</span>
      </div>
  ...
  <div class="collection-bar" id="collection-bar"></div>
...
<div class="unlock-overlay" id="unlock-overlay">
  <div class="unlock-card" id="unlock-card">
    <div class="unlock-badge">NEW FRIEND</div>
    <div class="unlock-mascot-wrap" id="unlock-mascot"></div>
    <div class="unlock-name" id="unlock-name"></div>
    <div class="unlock-desc" id="unlock-desc"></div>
    <div class="unlock-time" id="unlock-time"></div>
    <button class="unlock-btn" id="unlock-close">만나서 반가워!</button>
  </div>
</div>
...
async function poll() {
  try {
    ...
    // Check for session state transitions (for toasts)
    const now = Date.now();
    sessions.forEach(s => {
      const age = (now - new Date(s.updated?.replace(" ","T")).getTime()) / 60000;
      s._ds = (age > 10 && s.status !== "waiting") ? "stale" : s.status;
    });
    checkSessionTransitions(sessions);
    render(sessions);
...
function spawnRoamingPet(layer, maxX) {
  ...
  const unlockedArr = [...getUnlockedSet()];
  const charIdx = unlockedArr[Math.floor(Math.random() * unlockedArr.length)];
  pet.innerHTML = '<div class="pet-shadow"></div>' +
...
function openPicker(sid) {
  ...
  grid.innerHTML = CHAR_NAMES.map((name, i) => {
    ...
    const m = isLocked ? lockedMascot(i) : mascot("waiting", "picker-" + i, i);
    const t = UNLOCK_THRESHOLDS.find(t => t.idx === i);
    const lockLabel = isLocked && t ? formatThresholdTime(t.minutes) : "";
    return '<div class="picker-item' + (i === pickerSelected ? ' selected' : '') + (isLocked ? ' locked' : '') + '" data-idx="' + i + '" ' + (isLocked ? 'style="opacity:0.45;cursor:not-allowed"' : '') + '>' +
      m + '<span class="picker-name">' + (isLocked ? lockLabel : name) + '</span></div>';
  }).join("");

  grid.querySelectorAll(".picker-item").forEach(item => {
    item.onclick = () => {
      const idx = parseInt(item.dataset.idx);
      if (!unlocked.has(idx)) return; // Can't select locked
...
const UNLOCK_THRESHOLDS = [
  { idx: 0,  minutes: 0,     name: "고양이", desc: "처음부터 함께하는 친구" },
  { idx: 1,  minutes: 0,     name: "강아지", desc: "처음부터 함께하는 친구" },
  { idx: 2,  minutes: 0,     name: "토끼",   desc: "처음부터 함께하는 친구" },
  { idx: 3,  minutes: 0,     name: "곰",     desc: "처음부터 함께하는 친구" },
  { idx: 4,  minutes: 30,    name: "펭귄",   desc: "30분 사용 달성! 시원한 친구가 도착했어요" },
  { idx: 5,  minutes: 60,    name: "호랑이", desc: "1시간 사용 달성! 용맹한 친구를 만났어요" },
  { idx: 6,  minutes: 120,   name: "사자",   desc: "2시간 사용! 왕의 귀환이에요" },
  { idx: 7,  minutes: 300,   name: "알파카", desc: "5시간 사용! 뽀글뽀글 친구가 왔어요" },
  { idx: 8,  minutes: 480,   name: "여우",   desc: "8시간 사용! 영리한 친구가 나타났어요" },
  { idx: 9,  minutes: 720,   name: "햄스터", desc: "12시간 사용! 바쁜 친구가 합류했어요" },
  { idx: 10, minutes: 1200,  name: "부엉이", desc: "20시간 사용! 현명한 친구를 만났어요" },
  { idx: 11, minutes: 3000,  name: "판다",   desc: "50시간 사용! 전설의 친구가 나타났어요!" },
];

const USAGE_KEY = "claude-paws-usage";
const UNLOCKED_KEY = "claude-paws-unlocked";

function getUsageMinutes() {
  try { return parseFloat(localStorage.getItem(USAGE_KEY) || "0"); } catch { return 0; }
}
function setUsageMinutes(m) {
  localStorage.setItem(USAGE_KEY, String(m));
}
function getUnlockedSet() {
  try { return new Set(JSON.parse(localStorage.getItem(UNLOCKED_KEY) || "[0,1,2,3]")); } catch { return new Set([0,1,2,3]); }
}
...
function checkUnlocks() {
  const minutes = getUsageMinutes();
  const unlocked = getUnlockedSet();
  const newUnlocks = [];

  for (const t of UNLOCK_THRESHOLDS) {
    if (minutes >= t.minutes && !unlocked.has(t.idx)) {
      unlocked.add(t.idx);
      newUnlocks.push(t);
    }
  }
  ...
  if (newUnlocks.length > 0) {
    ...
    // Show celebration for the first new unlock (queue if multiple)
    showUnlockQueue(newUnlocks);
  }

  renderCollectionBar(minutes, unlocked);
}

let unlockQueue = [];
function showUnlockQueue(newUnlocks) {
  unlockQueue.push(...newUnlocks);
  if (unlockQueue.length === newUnlocks.length) showNextUnlock();
}
function showNextUnlock() {
  if (unlockQueue.length === 0) return;
  const t = unlockQueue.shift();
  showUnlockCelebration(t);
}

function showUnlockCelebration(threshold) {
  const overlay = document.getElementById("unlock-overlay");
  const card = document.getElementById("unlock-card");
  document.getElementById("unlock-mascot").innerHTML = mascot("waiting", "unlock-" + threshold.idx, threshold.idx);
  document.getElementById("unlock-name").textContent = threshold.name;
  document.getElementById("unlock-desc").textContent = threshold.desc;

  const hrs = Math.floor(threshold.minutes / 60);
  const mins = threshold.minutes % 60;
  const timeStr = hrs > 0 ? (hrs + "시간" + (mins > 0 ? " " + mins + "분" : "")) : (mins + "분");
  document.getElementById("unlock-time").textContent = threshold.minutes > 0 ? ("누적 사용 시간: " + timeStr) : "";

  // Spawn confetti
  const confettiColors = ["#3182f6","#20c997","#f59f00","#7048e8","#ff6b6b","#ffd43b","#69db7c"];
  const confettiEls = [];
  for (let i = 0; i < 24; i++) {
  ...
  setTimeout(() => confettiEls.forEach(c => c.remove()), 3000);
}

document.getElementById("unlock-close").onclick = () => {
  document.getElementById("unlock-overlay").classList.remove("open");
  setTimeout(showNextUnlock, 300);
};
document.getElementById("unlock-overlay").onclick = (e) => {
  if (e.target === document.getElementById("unlock-overlay")) {
    document.getElementById("unlock-overlay").classList.remove("open");
    setTimeout(showNextUnlock, 300);
  }
};

function renderCollectionBar(minutes, unlocked) {
  const bar = document.getElementById("collection-bar");
  const total = UNLOCK_THRESHOLDS.length;
  ...
  const dots = UNLOCK_THRESHOLDS.map(t => {
    const isUnlocked = unlocked.has(t.idx);
    const m = isUnlocked ? miniCollectionMascot(t.idx) : miniLockedMascot(t.idx);
    return '<div class="collection-dot ' + (isUnlocked ? "unlocked" : "locked") + '" title="' + t.name + (isUnlocked ? "" : " (" + formatThresholdTime(t.minutes) + ")") + '">' + m + '</div>';
  }).join("");

  bar.innerHTML =
    '<span class="collection-label">컬렉션</span>' +
    '<div class="collection-mini">' + dots + '</div>' +
    '<div class="collection-progress"><div class="collection-fill" style="width:' + pct + '%"></div></div>' +
    '<span class="collection-count">' + count + '/' + total + '</span>' +
    '<span class="collection-label" style="color:var(--gray-400)">' + timeStr + '</span>';
}

function lockedMascot(idx) {
  // Render the actual animal shape as a dark silhouette with a lock overlay
  const silhouetteColor = "var(--gray-300, #b0b8c1)";
  const chars = [
    { ear: '<polygon points="2,4 4,1 6,4" fill="' + silhouetteColor + '"/><polygon points="10,4 12,1 14,4" fill="' + silhouetteColor + '"/>' },
    { ear: '<rect x="1" y="3" width="3" height="5" rx="1.5" fill="' + silhouetteColor + '"/><rect x="12" y="3" width="3" height="5" rx="1.5" fill="' + silhouetteColor + '"/>' },
    { ear: '<rect x="4" y="0" width="2.5" height="5" rx="1.2" fill="' + silhouetteColor + '"/><rect x="9.5" y="0" width="2.5" height="5" rx="1.2" fill="' + silhouetteColor + '"/>' },
    { ear: '<circle cx="3.5" cy="3" r="2" fill="' + silhouetteColor + '"/><circle cx="12.5" cy="3" r="2" fill="' + silhouetteColor + '"/>' },
    { ear: '' },
    { ear: '<polygon points="2,4 4,1 6,4" fill="' + silhouetteColor + '"/><polygon points="10,4 12,1 14,4" fill="' + silhouetteColor + '"/>' },
    { ear: '<circle cx="3" cy="3.5" r="2.5" fill="' + silhouetteColor + '"/><circle cx="13" cy="3.5" r="2.5" fill="' + silhouetteColor + '"/><circle cx="8" cy="1.5" r="2" fill="' + silhouetteColor + '"/>' },
    { ear: '<ellipse cx="3.5" cy="3" rx="1.5" ry="2" fill="' + silhouetteColor + '"/><ellipse cx="12.5" cy="3" rx="1.5" ry="2" fill="' + silhouetteColor + '"/><circle cx="8" cy="2" r="2.5" fill="' + silhouetteColor + '"/>' },
    { ear: '<polygon points="2,4 4,0 6,4" fill="' + silhouetteColor + '"/><polygon points="10,4 12,0 14,4" fill="' + silhouetteColor + '"/>' },
    { ear: '<circle cx="3" cy="3.5" r="2" fill="' + silhouetteColor + '"/><circle cx="13" cy="3.5" r="2" fill="' + silhouetteColor + '"/>' },
    { ear: '<polygon points="3,4 4.5,1 6,4" fill="' + silhouetteColor + '"/><polygon points="10,4 11.5,1 13,4" fill="' + silhouetteColor + '"/>' },
    { ear: '<circle cx="3.5" cy="3" r="2" fill="' + silhouetteColor + '"/><circle cx="12.5" cy="3" r="2" fill="' + silhouetteColor + '"/>' },
  ];
  const ch = chars[idx] || chars[0];
  return '<div style="width:32px;height:32px;position:relative">' +
    '<svg viewBox="0 0 16 16" shape-rendering="crispEdges" style="width:32px;height:32px;opacity:0.5;filter:saturate(0)">' +
      ch.ear +
      '<rect x="3" y="3.5" width="10" height="7" rx="2" fill="' + silhouetteColor + '"/>' +
      '<rect x="5" y="5.5" width="1.5" height="1.5" rx="0.3" fill="var(--gray-400,#8b95a1)"/>' +
      '<rect x="9.5" y="5.5" width="1.5" height="1.5" rx="0.3" fill="var(--gray-400,#8b95a1)"/>' +
      '<rect x="4" y="10" width="8" height="4.5" rx="1.5" fill="' + silhouetteColor + '"/>' +
    '</svg>' +
    '<svg viewBox="0 0 16 16" fill="none" style="position:absolute;bottom:-1px;right:-1px;width:14px;height:14px">' +
      '<rect x="3" y="7" width="10" height="7" rx="1.5" fill="var(--gray-400,#8b95a1)"/>' +
      '<path d="M5 7V5.5a3 3 0 016 0V7" stroke="var(--gray-400,#8b95a1)" stroke-width="1.5" fill="none"/>' +
      '<circle cx="8" cy="10.5" r="1" fill="var(--gray-200,#e5e8eb)"/>' +
    '</svg>' +
  '</div>';
}

function miniCollectionMascot(idx) {
  const colors = ["#FFD699","#C4956A","#F5E6D3","#A0785A","#2D3436","#F4A623","#E8A640","#F5F0E8","#E87826","#F0C78A","#8B6544","#F5F5F5"];
  const col = colors[idx] || "#C4956A";
  return '<svg viewBox="0 0 16 16" shape-rendering="crispEdges"><rect x="3" y="3.5" width="10" height="7" rx="2" fill="' + col + '"/><rect x="5" y="5.5" width="1.5" height="1.5" rx="0.3" fill="var(--gray-900,#191f28)"/><rect x="9.5" y="5.5" width="1.5" height="1.5" rx="0.3" fill="var(--gray-900,#191f28)"/><rect x="4" y="10" width="8" height="4.5" rx="1.5" fill="' + col + '"/></svg>';
}

function miniLockedMascot(idx) {
  const colors = ["#FFD699","#C4956A","#F5E6D3","#A0785A","#2D3436","#F4A623","#E8A640","#F5F0E8","#E87826","#F0C78A","#8B6544","#F5F5F5"];
  const col = colors[idx] || "#C4956A";
  return '<svg viewBox="0 0 16 16" shape-rendering="crispEdges" style="filter:saturate(0) brightness(0.8)"><rect x="3" y="3.5" width="10" height="7" rx="2" fill="' + col + '"/><rect x="4" y="10" width="8" height="4.5" rx="1.5" fill="' + col + '"/></svg>';
}

function formatThresholdTime(min) {
  if (min === 0) return "시작";
...
// Track usage time - increment every 30 seconds if page is visible
let usageTracker = setInterval(() => {
  if (!document.hidden) {
    const current = getUsageMinutes();
    setUsageMinutes(current + 0.5); // 30 seconds = 0.5 minutes
    checkUnlocks();
  }
}, 30000);
...
const NOTIF_KEY = "claude-paws-notifications";
...
function updateNotifUI(enabled) {
  document.getElementById("notifTrack").classList.toggle("on", enabled);
  document.getElementById("notifLabel").textContent = enabled ? "알림" : "알림 끔";
}

document.getElementById("notifToggle").onclick = () => {
  const current = getNotifEnabled();
  setNotifEnabled(!current);
};

// Init notification toggle
updateNotifUI(getNotifEnabled());
// Sync initial state to server
fetch("/api/config/notifications", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ enabled: getNotifEnabled() }),
}).catch(() => {});
...
function showToast(title, desc, type) {
  ...
  const iconCls = type === "done" ? "done" : "alert";
  const iconChar = type === "done" ? "V" : "!";
  const now = new Date();
  const timeStr = now.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit" });

  toast.innerHTML =
    '<div class="toast-icon ' + iconCls + '">' + iconChar + '</div>' +
    '<div class="toast-body"><div class="toast-title">' + esc(title) + '</div><div class="toast-desc">' + esc(desc) + '</div></div>' +
...
function checkSessionTransitions(sessions) {
  sessions.forEach(s => {
    const prev = prevSessionStatuses[s.session];
    const curr = s._ds || s.status;
    const name = s.sessionName || s.project || "unknown";

    if (prev && prev !== curr) {
      if (curr === "waiting" && prev === "working") {
        showToast(name, "작업 완료 - 입력 대기", "done");
      } else if (curr === "notification" || (s.status && s.status.startsWith("notification"))) {
        showToast(name, s.message || "알림", "alert");
      }
    }
    prevSessionStatuses[s.session] = curr;
  });
}
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/config/notifications" && req.method === "POST") {
    let body = "";

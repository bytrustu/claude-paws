...
function showToast(project, status, message, sid) {
  ...
  // Hover: show arrow only on mouseenter
  toast.addEventListener("mouseenter", () => {
    const c = document.querySelector('[data-sid="' + sid + '"]');
    if (!c) return;
    const svg = document.getElementById("toast-arrows");
    if (!svg) return;
    // Remove any existing hover arrow
    ...
    if (req.method === "GET") {
      ...
      try {
        ...
        try {
          ...
          const hookSrc = join(fileURLToPath(import.meta.url), "..", "hooks", "session-tracker.sh");
          const hookDir = join(homedir(), ".claude", "hooks");
          const hookDst = join(hookDir, "session-tracker.sh");

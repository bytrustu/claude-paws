...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .btn-focus-terminal:hover { background: var(--blue-bg); color: var(--blue); border-color: var(--blue); }
  .btn-focus-terminal svg { width: 14px; height: 14px; stroke: currentColor; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
  .modal-body {
    flex: 1;
    overflow-y: auto;
...
<div class="modal-overlay" id="modal" style="display:none">
  <div class="modal">
    ...
    <div class="modal-actions">
      <button class="btn-focus-terminal" id="btn-focus-terminal" style="display:none">
...
async function openModal(sid) {
  try {
    ...
    const focusBtn = document.getElementById("btn-focus-terminal");
    if (data.tty) {
      focusBtn.style.display = "inline-flex";
      focusBtn.onclick = async () => {
        try {
          const r = await fetch(\`/api/session/\${sid}/focus\`, { method: "POST" });
          const result = await r.json();
          if (!result.ok) console.error("Focus failed:", result.error);
  ...
  if (url.pathname === "/api/sessions") {
    ...
    if (!sid || sid.includes("..") || sid.includes("/")) {
      ...
      try {
        ...
        let script;
        if (termApp === "Warp") {
          // Warp: activate app + send bell to TTY to highlight tab
          // Warp doesn't expose tabs via Accessibility API (GPU-rendered custom UI)
          ...
          // Fallback: try to find any terminal app and activate it
          script = [
            'tell application "System Events" to set appList to name of every application process',
            'if appList contains "Warp" then',
            '  tell application "Warp" to activate',
            '  return "found"',
            'else if appList contains "iTerm2" then',
        ...
        try {
          const result = execSync(`osascript "${scriptPath}"`, { timeout: 5000 }).toString().trim();
          res.writeHead(200, { "Content-Type": "application/json" });

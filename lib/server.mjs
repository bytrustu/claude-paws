...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .btn-delete-session { border: 1px solid var(--gray-200); background: none; color: var(--gray-500); font-size: 12px; font-weight: 500; padding: 4px 10px; border-radius: 6px; cursor: pointer; font-family: inherit; transition: all .15s; }
  .btn-delete-session:hover { border-color: #e03131; color: #e03131; background: #fff5f5; }
  .modal-info {
    padding: 16px 28px;
    display: flex;
    gap: 20px;
...
<div class="modal-overlay" id="modal" style="display:none">
  <div class="modal">
    <div class="modal-header">
      ...
      <button class="btn-delete-session" id="btn-delete-session">세션 삭제</button>
...
// Map of sid -> { toast el, count, timerId }
const toastGroups = new Map();
const TOAST_MAX = 5;
const TOAST_LIFETIME = 30000;
...
function showToast(project, status, message, sid) {
  ...
  if (existing) {
    ...
    existing.timerId = setTimeout(() => dismissToast(sid), TOAST_LIFETIME);
    ...
    if (oldTimer) {
      ...
      newTimer.className = "toast-timer";
      toast.appendChild(newTimer);
    }
  ...
  const timerId = setTimeout(() => dismissToast(sid), TOAST_LIFETIME);
  toastGroups.set(sid, { toast, count: 1, timerId });
